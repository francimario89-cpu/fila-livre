...
import { Bell, Building2, User, MapPin, LayoutGrid, CloudSync, Wifi } from 'lucide-react';
...
export const Layout: React.FC<LayoutProps> = ({ 
  ...
  // Simulação de sincronização com banco de dados Google (Firebase)
  useEffect(() => {
      ...
      <header className="p-4 flex items-center justify-between sticky top-0 z-50 glass-card border-b border-white/5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <div className={`flex items-center gap-1 px-1.5 py-0.5 rounded-full bg-slate-900 border border-white/5 ${syncStatus === 'syncing' ? 'animate-pulse' : ''}`}>
                <Wifi size={8} className={syncStatus === 'syncing' ? 'text-amber-500' : 'text-emerald-500'} />
                <span className="text-[6px] font-black uppercase tracking-tighter text-slate-500">Cloud Sync</span>
              </div>
            </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className={`w-9 h-9 rounded-xl p-[1.5px] shadow-lg ${
            userRole === 'admin' ? 'bg-gradient-to-tr from-indigo-600 to-indigo-400' : 'bg-gradient-to-tr from-teal-600 to-teal-400'
          }`}>
            ...
            return (
              <button
                ...
                className={`flex flex-col items-center gap-1.5 flex-1 py-3 rounded-[24px] transition-all duration-400 ${
                  isActive 
                    ? userRole === 'admin' ? 'bg-indigo-600 text-white shadow-xl scale-105' : 'bg-teal-500 text-slate-950 shadow-xl scale-105' 
