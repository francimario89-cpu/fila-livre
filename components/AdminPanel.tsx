...
import { 
  Plus, Trash2, BarChart3, Users2, UserCircle, Clock, Zap, Gift, Lock, Timer, FileText, Store, AlertTriangle, Save, ToggleLeft, ToggleRight
} from 'lucide-react';
import { Professional, Service, QueueItem, EstStatus, BookingModel, RevenueRecord, PlanType, ServiceRating, Establishment } from '../types';
...
export const AdminPanel: React.FC<AdminPanelProps> = ({
  establishment, queue, services, professionals, estStatus, bookingModel, plan, trialStartedAt, loyaltyEnabled, revenue, pixKey,
    ...
    <div className="space-y-8 pb-20 animate-in fade-in duration-500">
      ...
      <section className="space-y-4">
        <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2">
          <BarChart3 size={14} className="text-emerald-500" /> Faturamento Total
        </h3>
        <div className="bg-slate-900 border border-slate-800 p-6 rounded-[32px] space-y-4 shadow-xl">
          ...
          <button onClick={() => setIsFinancialModalOpen(true)} className="w-full bg-slate-950 border border-slate-800 py-4 rounded-2xl text-[10px] font-black uppercase text-slate-300 flex items-center justify-center gap-2">
            <FileText size={16} className="text-emerald-500" /> Relatório Semanal/Mensal
          </button>
        </div>
      </section>
      ...
      <section className="space-y-4">
        <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2">
          <Gift size={14} className="text-amber-500" /> Programa de Fidelidade
        </h3>
        <button 
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="text-left">
              ...
              <p className="text-[9px] text-slate-500 font-bold uppercase mt-0.5">
                {loyaltyEnabled ? 'Clientes ganham selos por corte' : 'Ative para oferecer cortes grátis'}
              </p>
            </div>
          </div>
      ...
      <section className="space-y-4">
        <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2">
          <Users2 size={14} className="text-indigo-400" /> Equipe
        </h3>
        <div className="space-y-3">
          {professionals.map(p => (
            <div key={p.id} className="bg-slate-900 border border-slate-800 p-5 rounded-[32px] flex items-center justify-between">
          ...
          {!isAddingPro ? (
            ...
            <div className="bg-slate-900 border border-slate-800 p-6 rounded-[32px] space-y-4 animate-in slide-in-from-top-4">
              <input autoFocus placeholder="NOME" value={newProName} onChange={e => setNewProName(e.target.value.toUpperCase())} className="w-full bg-slate-950 border border-slate-800 rounded-2xl px-5 py-4 text-xs text-white uppercase font-black" />
              <div className="flex gap-2">
                ...
                <button onClick={() => { if(newProName) { onUpdatePros([...professionals, {id: Math.random().toString(36).substr(2, 9), name: newProName, status: 'available', establishmentId: establishment.id}]); setIsAddingPro(false); setNewProName(''); } }} className="flex-1 bg-indigo-600 text-white py-4 rounded-2xl font-black text-[10px] uppercase">Confirmar</button>
              </div>
            </div>
          )}
        </div>
      ...
      <section className="space-y-4">
        ...
        <div className="space-y-3">
          {services.map(s => (
            <div key={s.id} className="bg-slate-900 border border-slate-800 p-4 rounded-[28px] flex items-center justify-between">
              <div className="flex items-center gap-4">
          ...
          {!isAddingService ? (
            ...
            <div className="bg-slate-900 border border-slate-800 p-6 rounded-[32px] space-y-4 animate-in slide-in-from-top-4">
              <input autoFocus placeholder="NOME DO SERVIÇO" value={newServiceName} onChange={e => setNewServiceName(e.target.value.toUpperCase())} className="w-full bg-slate-950 border border-slate-800 rounded-2xl px-5 py-4 text-xs text-white uppercase font-black" />
              <input type="number" placeholder="PREÇO" value={newServicePrice} onChange={e => setNewServicePrice(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-2xl px-5 py-4 text-xs text-white font-black" />
              <div className="flex gap-2">
                ...
                <button onClick={() => { if(newServiceName && newServicePrice) { onUpdateServices([...services, {id: Math.random().toString(36).substr(2, 9), name: newServiceName, price: newServicePrice, duration: parseInt(newServiceDuration), establishmentId: establishment.id}]); setIsAddingService(false); setNewServiceName(''); setNewServicePrice(''); } }} className="flex-1 bg-teal-500 text-slate-950 py-4 rounded-2xl font-black text-[10px] uppercase">Confirmar</button>
              </div>
            </div>
          )}
        </div>
      ...
      <section className="space-y-4 pt-10">
        <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2">
          <Store size={14} className="text-indigo-400" /> Configurações
        </h3>
        <div className="bg-slate-900 border border-slate-800 rounded-[32px] p-8 space-y-6">
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => onUpdateEstablishment({ name: editEstName.toUpperCase() })} className="bg-indigo-600 text-white p-4 rounded-2xl shadow-lg"><Save size={18} /></button>
            </div>
          </div>
          <div className="pt-6 border-t border-white/5">
             <button onClick={onDeleteEstablishment} className="w-full bg-red-500/10 border border-red-500/20 text-red-500 py-5 rounded-2xl text-[10px] font-black uppercase shadow-xl">Excluir Unidade Permanentemente</button>
          </div>
        </div>
      </section>
