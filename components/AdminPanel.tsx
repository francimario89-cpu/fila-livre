...
import { 
  Plus, Trash2, BarChart3, Users2, UserCircle, Clock, Zap, Gift, FileText, Store, Save, ToggleLeft, ToggleRight, Monitor, UserPlus, Coffee, DoorClosed, CheckCircle2, UserMinus, Timer, Scissors, Calendar, ListOrdered, Settings, QrCode
} from 'lucide-react';
    ...
    <div className="space-y-8 pb-24 animate-in fade-in duration-500">
      ...
      <section className="space-y-4">
        <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2">
          <Store size={14} className="text-indigo-400" /> Operação de Hoje
        </h3>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <button onClick={() => setIsAddingManual(true)} className="bg-indigo-600 p-6 rounded-[32px] flex items-center justify-center gap-3 shadow-xl shadow-indigo-500/20 active:scale-95 transition-all">
            ...
            <span className="text-[10px] font-black text-white uppercase tracking-widest">Add na Fila</span>
          </button>
        </div>
      </section>
      ...
      <section className="space-y-4">
        <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2">
          <BarChart3 size={14} className="text-emerald-500" /> Financeiro & Pagamentos
        </h3>
        <div className="bg-slate-900 border border-slate-800 rounded-[32px] p-6 space-y-6">
           <div className="flex items-center justify-between">
              <div>
                <p className="text-[9px] text-slate-500 font-black uppercase">Faturamento Total</p>
                <h4 className="text-2xl font-black text-white">R$ {totalEarnings.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</h4>
              </div>
              <button onClick={() => setIsFinancialModalOpen(true)} className="p-3 bg-slate-800 rounded-2xl text-emerald-400">
                <FileText size={20} />
              </button>
           </div>
           
           <div className="space-y-2">
             <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest ml-1">Sua Chave PIX (Para Clientes)</label>
             <div className="relative">
                <QrCode className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-700" size={18} />
                <input 
                  value={pixKey}
                  onChange={(e) => onSetPixKey(e.target.value)}
                  placeholder="E-mail, CPF ou Aleatória"
                  className="w-full bg-slate-950 border border-slate-800 rounded-2xl py-4 pl-12 pr-4 text-white text-xs font-bold outline-none focus:border-emerald-500 transition-all"
                />
             </div>
           </div>
        </div>
          ...
          <div className="p-6 border-b border-slate-800 space-y-4">
            ...
            <div className="grid grid-cols-3 gap-2">
              {[
                ...
                { id: 'appointment', label: 'Só Agendar' },
                { id: 'both', label: 'Ambos' }
              ].map(m => (
                <button 
                  key={m.id} 
          ...
          <div className="p-6 flex items-center justify-between">
             <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col">
                  <span className="text-[10px] font-black text-white uppercase">Cartão Fidelidade</span>
                  <span className="text-[7px] font-bold text-slate-500 uppercase mt-1">{loyaltyEnabled ? 'Ativado (Corte Grátis)' : 'Desativado'}</span>
                </div>
             </div>
             <button 
      ...
      <section className="space-y-4">
        ...
        <div className="grid grid-cols-1 gap-3">
          {professionals.map(p => (
            <div key={p.id} className="bg-slate-900 border border-slate-800 p-5 rounded-[32px] space-y-4 shadow-xl">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
        ...
        <div className="fixed inset-0 z-[400] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-sm bg-slate-900 border border-white/10 p-8 rounded-[40px] space-y-6">
            ...
            <div className="space-y-4">
              <input placeholder="NOME DO CLIENTE" value={manualName} onChange={e => setManualName(e.target.value.toUpperCase())} className="w-full bg-slate-950 border border-slate-800 rounded-2xl p-4 text-xs font-bold text-white uppercase outline-none" />
              <select value={manualService} onChange={e => setManualService(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-2xl p-4 text-xs font-bold text-white uppercase outline-none">
                {services.map(s => <option key={s.id} value={s.name}>{s.name}</option>)}
              </select>
            </div>
            <button onClick={() => { if(!manualName) return; onManualJoin({ name: manualName, service: manualService, professionalId: 'any', type: 'walk-in' }); setManualName(''); setIsAddingManual(false); }} className="w-full bg-indigo-600 text-white p-4 rounded-2xl font-black text-[10px] uppercase shadow-lg shadow-indigo-500/20">Inserir na Fila</button>
          </div>
        </div>
      )}
