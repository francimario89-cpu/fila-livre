  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (err.code === 'auth/operation-not-allowed') {
        setError('O Login Google não está ativado no Firebase.');
      } else if (err.code === 'auth/unauthorized-domain') {
        setError('Domínio não autorizado.');
      } else {
        setError(`Falha na autenticação: ${err.code}`);
      }
    } finally {
      setIsLoading(false);
    }
        ...
        {activeTab === 'email' ? (
          <form onSubmit={handleEmailAuth} className="space-y-4">
            <input
              ...
              className="w-full bg-slate-900 border border-slate-800 rounded-2xl py-4.5 px-6 text-white text-sm outline-none focus:border-teal-500 transition-all"
            />
            <input
              required
              type="password"
            ...
            <button
              disabled={isLoading}
              type="submit"
              className={`w-full py-5 rounded-2xl font-black text-[10px] tracking-widest flex items-center justify-center gap-3 shadow-xl ${
                role === 'admin' ? 'bg-indigo-600 text-white shadow-indigo-600/20' : 'bg-teal-500 text-slate-950 shadow-teal-500/20'
              }`}
            >
              {isLoading ? <Loader2 className="animate-spin" size={18} /> : (isRegistering ? "CONFIRMAR CADASTRO" : "ENTRAR")}
            </button>
          ...
          <button 
            ...
            className="w-full bg-white text-slate-950 py-5 rounded-2xl flex items-center justify-center gap-3 text-[11px] font-black uppercase shadow-xl"
          >
