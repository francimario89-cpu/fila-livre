  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-32">
      ...
      {switchSuggestion && myQueueItem && (
        <div className="bg-amber-500 rounded-[32px] p-6 shadow-2xl animate-bounce-subtle flex items-center justify-between gap-4">
           <div className="flex items-center gap-4">
              <div className="w-12 h-12 bg-white/20 rounded-2xl flex items-center justify-center text-slate-950"><Scissors size={24} /></div>
              <div>
           ...
           <button onClick={() => onSwitchQueue?.(myQueueItem.id, switchSuggestion.id)} className="bg-slate-950 text-white px-4 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest shadow-lg flex items-center gap-2">Mudar Fila <ArrowRight size={14} /></button>
        </div>
      )}
      ...
      <div className={`p-5 rounded-[32px] border-2 flex items-center justify-between shadow-lg transition-all ${estStatus === 'open' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-500' : estStatus === 'lunch' ? 'bg-amber-500/10 border-amber-500/20 text-amber-500' : 'bg-red-500/10 border-red-500/20 text-red-500'}`}>
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[8px] font-bold uppercase opacity-60 tracking-tighter">{estStatus === 'open' ? (filterPro === 'all' ? 'Todas as filas disponíveis' : `Fila de ${professionals.find(p => p.id === filterPro)?.name}`) : 'Volte mais tarde'}</p>
          </div>
        </div>
      ...
      <section className="space-y-4">
        <h2 className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-3">{filterPro === 'all' ? 'Próximos na Fila Geral' : `Próximos para ${professionals.find(p => p.id === filterPro)?.name}`}</h2>
